...
namespace Ct
{
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice(Ref<GraphicsContext<RenderingAPI::Vulkan>> context)
	{
		...
		CT_ASSERT(!(deviceCount == 0), "Failed to find a GPU with Vulkan support!");
		...
		CT_ASSERT(m_PhysicalDevice, "Failed to find a GPU with support for this application's required Vulkan capabilities!");
	...
	bool VulkanPhysicalDevice::ExtensionsSupported(const VkPhysicalDevice& device)
	{
		...
		std::set<std::string> requiredExtensions(GraphicsContext<RenderingAPI::Vulkan>::s_RequestedDeviceExtensions.begin(), GraphicsContext<RenderingAPI::Vulkan>::s_RequestedDeviceExtensions.end());
